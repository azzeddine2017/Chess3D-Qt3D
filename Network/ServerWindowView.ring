...
if IsMainSourceFile() 
	new Qapp {
			StyleFusion()
			new ServerWindowView { win.show() } 
			exec()
		   }
ok

Class  ServerWindowView  from  WindowsViewParent 


					 Win = new  MainWindow() {  
								setWindowTitle("Connecting...")
		                        setstylesheet("color:#28ff02;background-image: url(C:/ring/game engine/qt_3d_chess/Qt3d_chess_core/textures/SpaceLightblue.png);")
						...
						Buttonok = new Qpushbutton(Win) {
								...
								setfont(oFont)
								oFont.delete()	
								setClickEvent(method(:startServer))	
								}
						ButtonCancel = new Qpushbutton(Win) {
								...
								setfont(oFont)
								oFont.delete()	
								setClickEvent(method(:cancelServer))	
								}
